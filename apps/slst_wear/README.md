...
- Main SLST app signed in to create a pairing code (Settings → Pair Watch)

## Configure

Uses the same public Supabase URL + anon key defaults as the main app
(`packages/slst_shared`). Override if needed:
...
```powershell
.\.tools\flutter\bin\flutter.bat -C apps\slst_wear pub get
.\.tools\flutter\bin\flutter.bat -C apps\slst_wear run -d <wear-device>
```

## Pairing flow

1. On Windows or Android SLST: Settings → **Pair Watch** → generate code.
...
- Shared domain/ops live in `packages/slst_shared` (`ShipOperations`,
