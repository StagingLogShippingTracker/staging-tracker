  ...
  if($('#tblShipped')) {
    ...
    if(shBody) {
      ...
      fShipped.slice(0, limitShipped).forEach(o => {
        ...
        // ADDED BATCH CHECKBOX LOGIC HERE
        const batchChk = `<input type="checkbox" style="width:18px;height:18px;" onchange="window.toggleBatchSelect('${o.id}', this.checked)" ${batchSelectedIds.has(o.id) ? 'checked' : ''}>`;

        shBody.insertAdjacentHTML('beforeend', `<tr ${rowClass}>
          <td class="show-in-batch" style="text-align:center;">${batchChk}</td>
          <td class="hide-in-batch">${editBtn}</td>
          <td class="hide-in-batch">${picBtn}</td>
          <td><a class="so-link" onclick="event.stopPropagation(); window.openOrderHistory('${o.so}')">${o.so}</a></td>
          <td>${o.customer}</td><td>${o.type}</td><td><b>${o.carrier || '—'}</b></td><td>${o.location}</td><td><small>${geoLink}</small></td>
          <td>${o.weight || '—'}</td><td class="hide-in-batch">${commentBtn}</td><td>${new Date(o.shipped_at).toLocaleString()}</td><td>${o.shipped_by || '—'}</td><td>${o.pmd_email ? o.pmd_email+(isRet?'':'<span class="green-check"> ✓</span>') : '—'}</td></tr>`);
      });
    }
  }
