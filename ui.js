    ...
    if(sBody) {
      ...
      fStaging.slice(0, limitStaging).forEach(o => {
        ...
        sBody.insertAdjacentHTML('beforeend', `<tr>
          ...
          <td>${o.weight || '—'}</td><td class="hide-in-batch">${commentBtn}</td><td style="color:${window.getStatusColor(o.status)}; font-weight:bold;">${window.getFormattedStatus(o.status)}</td><td>${o.staged_by||'—'}</td>
