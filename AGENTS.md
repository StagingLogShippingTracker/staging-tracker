...
- `packages/swift_staging_shared/` — shared domain/data used by Windows, Android, and Wear
  - `src/shared_name_directory.dart` — cross-**app** "remembered name" directories: `SharedContactsClient` (`shared_contacts` / `shared_contact_tombstones` — person names: "Staged By"/"Shipped By"/"Picked by"/"Returned by" here, Swift Document Generator's "Swift Contact" field, Wear's "Shipped By") and `SharedCarriersClient` (`shared_carriers` / `shared_carrier_tombstones` — "Carrier" here, in Document Generator, and on Wear). One shared directory per concept, not per-app copies. Do not add another person-name or carrier store; wire new fields to these (or `lib/data/person_name_memory.dart` for the richer Windows/Android person-name UI with local caching + forget/tombstone support).
