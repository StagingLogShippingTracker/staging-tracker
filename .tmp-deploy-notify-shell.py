...
ARGS_PATH = ROOT / ".tmp-deploy-notify-shell.json"
TOKEN_KEY = (
...
def main() -> None:
    ...
    assert args["name"] == "notify-pm"
    assert args["verify_jwt"] is True
    assert args["project_id"] == "gdrpdiwykmnybmkadlrv"
    assert len(args["files"]) == 4
    combined = "".join(f["content"] for f in args["files"])
    for needle in [
        "Deno.serve",
        'const D_SHELL = "#2e3033"',
        'ASSET_VERSION = "20260725j"',
        ".email-container, .og-shell { background-color: ${D_SHELL}",
