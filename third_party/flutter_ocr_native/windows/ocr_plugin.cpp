...
void FlutterOcrNativePlugin::EnsureInitialized() {
  ...
  // WinRT and Windows.Media.Ocr in the plugin constructor caused a fail-fast
  // during application startup on current Windows 11 builds. Initialize the
  // native resources lazily, only when OCR is first requested.
...
void FlutterOcrNativePlugin::RecognizeFromBytes(
    ...
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
  std::thread(
      [this, bytes, result = std::move(result)]() mutable {
        winrt::init_apartment(winrt::apartment_type::multi_threaded);
        RecognizeFromBytesWorker(bytes, std::move(result));
      })
      .detach();
}

void FlutterOcrNativePlugin::RecognizeFromBytesWorker(
    const std::vector<uint8_t>& bytes,
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
  try {
    EnsureInitialized();
    if (!ocr_engine_) {
      result->Error("NOT_INITIALIZED", "Windows OCR language is not installed");
      return;
    }
