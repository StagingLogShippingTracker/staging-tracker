...
ARGS_PATH = ROOT / ".tmp-notify-pm-deploy-files.json"
TOKEN_KEY = (
    "mcpOAuth.secret.W3BsdWdpbi1zdXBhYmFzZS1zdXBhYmFzZTo6bY3BTY29wZTo"
    "cHJvZmlsZTpaR1ZtWVhWc2RBXSBtY3BfdG9rZW5z"
)

# Fix: use the correct key from the working script
TOKEN_KEY = (
...
def main() -> None:
    ...
    for bad in ["PLACEHOLDER", "LOAD_FROM_DISK_NEXT", "Thank you for using SLST", "#2a2a2c"]:
        if bad in combined:
            raise SystemExit(f"found {bad}")

    if not args.get("verify_jwt", False):
