...
/// Syncs person names across Windows & Android using the same Document
/// Generator tables (`shared_contacts` + `shared_contact_tombstones`).
class ContactSync {
  ContactSync(this.store, {http.Client? client})
      : _client = client ?? http.Client();

  final RememberedContactsStore store;
  ...
  Future<void> syncOnLaunch({Iterable<String> seedNames = const []}) async {
    if (store.names.isEmpty && seedNames.isNotEmpty) {
      final seen = <String>{};
      final seeded = <String>[];
      for (final raw in seedNames) {
        final name = raw.trim();
        ...
        seen.add(key);
        seeded.add(name);
        if (seeded.length >= RememberedContactsStore.maxRememberedContacts) {
          break;
        }
      }
      store.names = seeded;
    ...
    for (final row in refreshed) {
      ...
      next.add(row.name);
      if (next.length >= RememberedContactsStore.maxRememberedContacts) break;
    }
    store.names = next;
    await store.save();
  }
  ...
  Future<bool> remember(String raw) async {
    ...
    final localChanged = await store.remember(name);
    try {
  ...
  Future<bool> forget(String raw) async {
    ...
    if (name.isEmpty) return false;
    final key = nameKey(name);
    final localChanged = await store.forget(name);
    try {
