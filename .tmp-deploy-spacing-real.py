"""MCP session init + deploy_edge_function with real notify-pm files (spacing fix)."""
...
ARGS_PATH = ROOT / ".tmp-mcp-final-args.json"
TOKEN_KEY = (
...
def main() -> None:
    ...
    assert "20260725l" in combined
    assert "width: 49%" in combined
    assert "max-width: 4px" in combined
    assert "Deno.serve" in combined
